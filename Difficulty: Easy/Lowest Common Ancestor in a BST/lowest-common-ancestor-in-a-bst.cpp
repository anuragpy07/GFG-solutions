...
public:
    Node* LCA(Node* root, Node* n1, Node* n2) {
        ...
        // If both values are smaller than root's data,
        // LCA must be in left subtree
        if (val1 < root->data && val2 < root->data) {
            return LCA(root->left, n1, n2);
        }
        
        // If both values are larger than root's data,
        // LCA must be in right subtree
        if (val1 > root->data && val2 > root->data) {
            return LCA(root->right, n1, n2);
        }
        
        // If one value is smaller and other is larger,
        // or if root matches either value,
